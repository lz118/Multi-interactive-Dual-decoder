...
class Decoder(nn.Module):
    ...
    def forward(self,rgb,t):
        xsize=rgb[0].size()[2:]
        global_info =self.global_info(rgb[3],t[3]) # 512 1/16
        d1=self.gfb4_1(global_info,t[3],rgb[2],global_info)
        d2=self.gfb4_2(global_info, rgb[3], t[2], global_info)
        #print(d1.shape,d2.shape)
        d3= self.gfb3_1(d1, d2,rgb[1],global_info)
        d4 = self.gfb3_2(d2, d1, t[1], global_info)
        d5 = self.gfb2_1(d3, d4, rgb[0], global_info)
        d6 = self.gfb2_2(d4, d3, t[0], global_info) #1/2 128
...
class Mnet(nn.Module):
    ...
    def forward(self,rgb,t):
        rgb_f=[]
        t_f = []
        x = self.rgb_net.layer1(self.rgb_net.maxpool(self.rgb_net.relu(self.rgb_net.bn1(self.rgb_net.conv1(rgb)))))
        ...
        x = self.t_net.layer1(self.t_net.maxpool(self.t_net.relu(self.t_net.bn1(self.t_net.conv1(t)))))
